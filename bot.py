...
# ---------- ЗЕРКАЛА ----------
mirrors = {}  # {token: {"bot": Bot, "dp": Dispatcher, "task": Task, "info": {}}}
...
def e(key: str) -> str:
    """Только премиум emoji"""
    pid = E.get(key, "")
    return f'<tg-emoji emoji-id="{pid}">.</tg-emoji>' if pid else "•"

# Для кнопок — только icon_custom_emoji_id, без текста эмодзи
...
async def create_mirror_task(token: str):
    """Запускает зеркало как asyncio.Task в том же event loop"""
    ...
    async def m_tiktok(msg: Message):
        ...
        try:
            ...
            await status.edit_text(f"<b>{e('cross')} Ошибка: {ex}</b>")
            return
        await status.delete()
    ...
    @mirror_dp.message(Command("mirror"))
    async def m_mirror_cmd(msg: Message, command: CommandObject):
        await msg.answer(f"<b>{e('info')} Зеркала создаются через основного бота.</b>")
    ...
    # Запускаем поллинг в том же event loop
...
async def cmd_mirror(msg: Message, command: CommandObject):
    ...
    await status.edit_text(
        ...
        f"{e('info')} Зеркало работает параллельно.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
